...
ndvi<-rast('D:/PhD/GIS/DP/NDVI/ndvi_desert_only.tif')
...
serie_ndvi<-terra::extract(ndvi,xy)%>%as.numeric()

df<-data.frame(date=date_table$date,ndvi=serie_ndvi)
...
write.csv(df,'D:/PhD/GIS/DP/NDVI/NDVI_TimeSerie_Csv.csv',row.names=FALSE)
...
ggplot(df, aes(x = date, y = ndvi)) +
  ...
  annotate("text", x = min(df$date), y = 0.17, hjust = 0, color = "blue",
           label = "Desert line (NDVI < 0.15)", size = 3) +
  theme_minimal() +
  labs(
    title = "NDVI Time Series at Desert Pixel (2020–2025)",
    x = "Date",
    y = "NDVI"
  ) +
...
ndvi_filtered<-serie_ndvi[mask_no_2023]
...
points(df_2023$doy,df_2023$ndvi,col='black',pch=16,cex=1.2)

# Time for anomaly detection using the ExtremeAnom Function

anom_ndvi<-ExtremeAnom(x=serie_ndvi,dates=date_table$date,h=2,refp=dates_filtered,anop=df_2023$date,rge = c(0,0.5),output='anomalies')
...
Anomalie<-ExtremeAnom(x=serie_ndvi,dates=date_table$date,refp = c(1,69),anop=c(70,99),rge=c(0,0.5),h=2,output='anomalies')
print(Anomalie)
...
ggplot(anomaly_df, aes(x = date, y = anomaly)) +
  ...
  labs(title = "NDVI Anomalies in 2023–2024",
       y = "NDVI Anomaly", x = "Date") +
  theme_minimal()

extreme_anom<-ExtremeAnom(x=serie_ndvi,dates=date_table$date,h=2,refp = c(1,69),anop=c(70,99),rge=c(0,0.5),output='clean',rfd=0.95)
